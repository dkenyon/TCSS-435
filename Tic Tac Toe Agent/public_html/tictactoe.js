...
//Checks to see if the cell is free or not
GameBoard.prototype.cellFree = function (cell) {
    if (this.X.indexOf(cell) < 0 && this.O.indexOf(cell) < 0) return true;
    else return false;
};

//Places either an 'X' or 'Y' in a lot
//Player turn then changes
//Then checks to see if game is over
GameBoard.prototype.move = function (cell) { //CELL IS AN INT
    if (this.cellFree(cell)) {
        //if it's player one's turn, push an 'X' into this cell
        //otherwise, push an 'O' into this cell
        this.playerOne ? this.X.push(cell) : this.O.push(cell);
        
        var c = this.cells[cell - 1];
        
        //if playerOne is true// className is X; O otherwise
        c.className = this.playerOne ? "X" : "O"; 
        //if playerOne is true, innerHTML is X; O otherwise
        c.innerHTML = this.playerOne ? "X" : "O"; 
        this.playerOne = !this.playerOne; //set playerOne to false
    }
...
//Returns 2 if O wins
//Returns 3 if draw
//Returns 0 if neither of those options (game keeps playing)
...
//Resets the board; clears all done moves and available cells to move in
